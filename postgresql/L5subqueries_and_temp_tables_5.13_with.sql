...
With t1 AS (SELECT s.name rep_name, r.name region, SUM(o.total_amt_usd) total
	FROM orders o
...
SELECT t1.rep_name, t1.region, t1.total
FROM t1
JOIN t2 ON t1.region = t2.region AND t1.total = total_amt


/* 2. For the region with the largest sales total_amt_usd, how many total orders were placed? */

WITH tb1 AS (SELECT r.name region, sum(o.total_amt_usd) total_amt
	FROM orders o
	JOIN accounts a ON o.account_id = a.id
	JOIN sales_reps s ON a.sales_rep_id = s.id
	JOIN region r ON s.region_id = r.id
...
/* 5. What is the lifetime average amount spent in terms of total_amt_usd for the top 10 
